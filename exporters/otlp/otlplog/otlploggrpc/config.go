...
func newConfig(options []Option) config {
	...
	c.path = c.path.Resolve(
		conf.GetEnv[string](envPathSignal, convPathExact),
		conf.GetEnv[string](envPathOTLP, convPath),
		conf.Fallback[string](defaultPath),
	)
...
func WithCompression(compression Compression) Option {
	return fnOpt(func(c config) config {
		c.compression = conf.NewSetting(compression)
		return c
	})
}
